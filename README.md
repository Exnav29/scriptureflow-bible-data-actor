...
The Actor supports three focused modes:
...
This Actor intentionally stays narrow. It does not provide random verse mode, book mode, chapter mode, full Bible export, bulk export, scraping, browser automation, API key gating, monetization logic, sermon generation, devotional generation, commentary generation, embeddings, or RAG chunks.

## What data can ScriptureFlow Bible Data return?

| Field | Description |
| --- | --- |
| `recordType` | Row type: `translation`, `verse`, `validation`, or `error`. |
| `mode` | Actor mode that produced the row: `catalog`, `passage`, or `validate_reference`. |
| `source` | Object describing ScriptureFlow as the provider, endpoint used, docs URL, CTA URL, and retrieval time. |
| `translationId` | ScriptureFlow translation ID, such as `en-kjv`. |
...
After the run finishes, read the default dataset items and use fields such as `text`, `reference`, `translationId`, `book`, `chapter`, and `verse` in later n8n nodes.

### AI-agent and MCP usage notes

This Actor is useful for agents that need structured Scripture lookup, reference validation, or catalog discovery, but it does not provide theological interpretation, sermon writing, devotional content, embeddings, or RAG chunks.
...
Default passage and validation runs are intentionally small. Catalog output can be bounded with `maxResults`.
...
* **`mode`** — one of `catalog`, `passage`, or `validate_reference`.
* **`translationId`** — ScriptureFlow translation ID, such as `en-kjv`; required for `passage` and `validate_reference`.
...
* **`includeMetadata`** — include non-text metadata when available.
* **`maxResults`** — maximum number of catalog rows to write.
...
```json
{
  ...
  "valid": false,
  "errorCode": "UNSUPPORTED_REFERENCE_FORMAT",
  "message": "Unsupported reference format.",
  "translationId": "en-kjv",
  "inputReference": "John 3:16; Romans 8:28",
  "status": null
}
```
...
* AI agents that need safe catalog discovery, reference validation, or structured passage lookup.
...
No. This Actor supports catalog lookup, passage lookup, and reference validation only. It does not provide full Bible export, book mode, chapter mode, random verse mode, or bulk export.
...
* `INVALID_MODE` — the requested mode is not supported. Use only `catalog`, `passage`, or `validate_reference`.
